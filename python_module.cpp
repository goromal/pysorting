...
PYBIND11_MODULE(pysorting, m) {
  ...
  m.def("validateState", &sorting::validateState,
        "Verify that the input state is formatted logically.");
  m.def("persistStateToDisk", &sorting::persistStateToDisk,
        "Persist the sorting state to disk. Reports success status.");
  m.def("sortStateFromDisk", &sorting::sortStateFromDisk,
        "Recover the sorting state from disk. Reports success status.");
  m.def("restfulRandomizedQuickSort", &sorting::restfulRandomizedQuickSort,
